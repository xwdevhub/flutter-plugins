...
namespace
{
      ...
      std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result)
  {
    ...
    else if (method_call.method_name() == "writeImage")
    {
      ...
      // string data = std::get<std::string>(arguments);

      // printVector(data1);
      // uint8_t *dat1 = data1.data();
      // const uint8_t *dat1 = &data1[0];

      // std::cout << data1.size() << std::endl;
      // std::cout << "aqq22" << std::endl;

      // uint8_t *dat1 = &data1.front();

      // std::cout << dat1 << std::endl;

      // size_t size = strlen((char *)(dat1));
