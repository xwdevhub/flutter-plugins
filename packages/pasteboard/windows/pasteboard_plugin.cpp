...
namespace {
    ...
    constexpr STGMEDIUM
    kNullStorageMedium = {TYMED_NULL, nullptr, nullptr};

    void printVector(const vector <uint8_t> &v) {
        cout << "vector content         : ";
        for_each(v.cbegin(), v.cend(), [](const uint8_t &val) -> void { cout << val; });
        cout << endl;
    }
    ...
    int GetDIBHeight(HANDLE hData) {
        ...
        if (bmi != NULL) {
            int height = abs(
                    bmi->bmiHeader.biHeight); // Make sure it's positive (DIB is upside-down)
            GlobalUnlock(hData);
            return height;
        }
    ...
    bool save2png(std::string filePath) {
        ...
        if (hBitmap != nullptr) {
            ...
            if (bitDepth == 32)
                dib_new = FreeImage_ConvertTo24Bits(dib_new);
            FreeImage_Save(FIF_PNG, dib_new, filePath.c_str());
            FreeImage_Unload(dib_new);

//            FIBITMAP* bmp_image = FreeImage_Load(FIF_BMP, filePath.c_str(), BMP_DEFAULT);
//            if (!bmp_image) {
//                return 1;
//            }
//            bmp_image = FreeImage_ConvertTo24Bits(bmp_image);
//            FreeImage_Save(FIF_PNG, bmp_image, (filePath+".png").c_str());
//
//            FreeImage_Unload(bmp_image);
